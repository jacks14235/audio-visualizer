...
get_background = lambda x, y, t, v, a:  (np.array(bg_grad.eval(((y/3) + t / 1000) % 1.0)) * 1).astype(np.int)
		...
		for x in range(N_BUCKETS):
			...
			for y in range(BUCKET_HEIGHT):
				if val > y / BUCKET_HEIGHT:
					...
					pixels[x, y] = get_background(x / N_BUCKETS, y / BUCKET_HEIGHT, curr, val, np.average(running_avg))
